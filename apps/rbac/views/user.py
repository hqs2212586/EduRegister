...
from EduRegister.settings import SECRET_KEY
...
class UserBuildMenuView(APIView):
    ...
    def get_menu_from_role(self, request):
        ...
        if request.user:
            menu_dict = {}
            menus = request.user.roles.values(      # values()返回一个ValueQuerySet,可迭代的字典序列
                'menus__id',
                'menus__name',
                'menus__path',
                'menus__is_frame',
                ...
                'menus__sort',
                'menus__pid'
            ).distinct()   # distinct：从返回结果中剔除重复记录
            for item in menus:
...
class UserViewSet(ModelViewSet):
    """用户管理：增删改查"""
    perms_map = (
        {'*': 'admin'}, {'*': 'user_all'}, {'get': 'user_list'}, {'post': 'user_create'},
        {'put': 'user_edit'}, {'delete': 'user_delete'}
    )
